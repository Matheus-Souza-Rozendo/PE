class HighResolutionTime {
  ...
  microsecondsToIso(microseconds) {
    ...
    const remainingUs = microseconds % 1000; // Parte fracionária → microssegundos restantes
    ...
    const microsecondsString = String(remainingUs).padStart(3, "0"); // Garante 3 dígitos

    return `${noMilliseconds}.${milliseconds}${microsecondsString}Z`; // Retorna no formato correto
}
  ...
  module.exports = HighResolutionTime;
